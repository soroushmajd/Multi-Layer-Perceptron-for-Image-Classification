...
model = MLP(units=units, hidden_layer_activation=args.activation, init_type=args.initialization)
...
for epoch in range(args.n_epochs):
    ...
    for i, batch in enumerate(train_dataloader):
        ...
        n_train_batches += 1
        temp = batch['image']
        images = batch['image'].reshape(-1, units[0])  # ([8, 12288])
    ...
    writer.add_scalar('Loss/train', train_running_loss / n_train_batches, epoch)
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(test_dataloader):
            n_test_batches += 1
            images = batch['image'].reshape(-1, units[0])
            labels = batch['label']
        ...
        writer.add_scalar('Loss/test', test_running_loss / n_test_batches, epoch)
